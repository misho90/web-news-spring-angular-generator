...
public class RSSFeedUtils {
  ...
  public static final List<RssFeedItemDto> fetchFeedItemsFromRssFeed(final Optional<SyndFeed> feed, final String sourceName) {
    if (feed.isPresent()) {
      ...
      return list.stream().map((rssFeed) -> new RssFeedItemDto(rssFeed.getTitle(), rssFeed.getLink(), rssFeed.getPublishedDate(),
          RSSFeedUtils.returnGuid(rssFeed.getLink()), sourceName)).collect(Collectors.toList());
    }
